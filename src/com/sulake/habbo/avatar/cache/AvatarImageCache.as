package com.sulake.habbo.avatar.cache
{
   ...
   public class AvatarImageCache
   {
      ...
      private var var_463:AvatarImageBodyPartContainer;
      ...
      private var var_2704:ByteArray;
      
      private var var_2705:ImageData;
      
      private var var_868:PaletteMap;
      ...
      public function setGeometryType(param1:String) : void
      {
         ...
         if(this.var_798 == GeometryType.const_1037 && param1 == GeometryType.const_1252 || this.var_798 == GeometryType.const_1252 && param1 == GeometryType.const_1037)
         {
            this.var_798 = param1;
            this.var_85 = null;
            return;
      ...
      public function getImageContainer(param1:String, param2:int) : AvatarImageBodyPartContainer
      {
            ...
            if(this.var_380.definition.isAnimation)
            {
               ...
               if(_loc12_ != null)
               {
                  ...
                  if(_loc14_ != null)
                  {
                     ...
                     if(_loc14_.type == AnimationLayerData.const_649)
                     {
                        if(_loc14_.action != null)
                        {
                     ...
                     else if(_loc14_.type == AnimationLayerData.const_492)
                     {
                        _loc3_ = _loc11_;
                     }
         ...
         if(_loc10_ == null)
         {
            _loc15_ = this._structure.getParts(param1,this.var_1446.getFigure(),_loc5_,this.var_798,_loc3_,_loc6_,_loc7_);
            _loc10_ = new AvatarImageDirectionCache(_loc15_);
            _loc9_.updateDirectionCache(_loc3_,_loc10_);
         }
         this.var_463 = _loc10_.getImageContainer(_loc4_);
         if(this.var_463 == null)
         {
            _loc16_ = _loc10_.getPartList();
            this.var_463 = this.renderBodyPart(_loc3_,_loc16_,_loc4_,this.var_380);
            if(this.var_463 == null)
            {
               return null;
            }
            _loc10_.updateImageContainer(this.var_463,_loc4_);
         }
         this.var_463.offset = _loc8_;
         return this.var_463;
      }
      ...
      private function renderBodyPart(param1:int, param2:Array, param3:int, param4:IActiveActionData) : AvatarImageBodyPartContainer
      {
         ...
         if(!this.var_85)
         {
            this.var_85 = this._structure.getCanvas(this._scale,this.var_798);
            if(!this.var_85)
            {
               return null;
            }
            this.var_2706 = new Point(0,this.var_85.height - int(this.var_85.width / 4));
         }
         var _loc5_:int = param1;
         var _loc6_:Boolean = AvatarDirectionAngle.const_1536[param1];
         var _loc9_:String = param4.definition.assetPartDefinition;
         var _loc10_:String = param4.definition.state;
         var _loc11_:Boolean = _loc10_ == AvatarAction.const_670 || _loc10_ == AvatarAction.const_395;
         ...
         while(_loc8_ >= 0)
         {
            ...
            {
               this.var_1496 = this.var_118.bodyPartId;
               this.var_190 = this.var_118.partType;
               this.var_1911 = this.var_118.partId;
               ...
               if(_loc6_)
               {
                  ...
                  else
                  {
                     ...
                     if(this.var_118.flippedPartType != this.var_190)
                     {
                        this.var_190 = this.var_118.flippedPartType;
                     }
                  }
               }
               this.var_1495 = this._scale + "_" + this.var_118.action.assetPartDefinition + "_" + this.var_190 + "_" + this.var_1911 + "_" + _loc5_ + "_" + this._frame;
               this.var_449 = this._assets.getAssetByName(this.var_1495) as BitmapDataAsset;
               if(this.var_449 == null)
               {
                  this.var_1495 = this._scale + "_std_" + this.var_190 + "_" + this.var_1911 + "_" + _loc5_ + "_0";
                  this.var_449 = this._assets.getAssetByName(this.var_1495) as BitmapDataAsset;
               }
               if(!(!this.var_449 || !this.var_449.content))
               {
                  if(this.var_118.paletteMapId > -1)
                  {
                     this.var_868 = this._structure.renderManager.paletteManager.getPalette(this.var_118.paletteMapId);
                     if(this.var_868 != null)
                     {
                        this.var_997 = this.var_449.content as BitmapData;
                        this.var_189 = new BitmapData(this.var_997.width,this.var_997.height);
                        this.var_2704 = this.var_868.colorize(this.var_997.getPixels(this.var_997.rect));
                        this.var_189.setPixels(this.var_189.rect,this.var_2704);
                     }
                     else
                     {
                        this.var_189 = (this.var_449.content as BitmapData).clone();
                     }
                  }
                  else
                  {
                  ...
                  if(_loc11_ && (this.var_1496 == AvatarBodyPartType.const_236 || this.var_1496 == AvatarBodyPartType.const_1563 || this.var_1496 == AvatarBodyPartType.const_1470) && this.var_118.color != null)
                  {
                     _loc16_ = 0.7;
                  ...
                  {
                     this.var_189.colorTransform(this.var_189.rect,this.var_118.color.colorTransform);
                  }
                  if(this.var_118.isBlendable)
                  {
                     this.var_189.colorTransform(this.var_189.rect,this.var_118.blendTransform);
                  }
                  this.var_996 = this.var_449.offset.clone();
                  if(_loc7_)
                  {
                     if(this._scale == AvatarScaleType.const_57)
                     {
                  ...
                  this.var_2705 = new ImageData(this.var_189,this.var_996,_loc7_);
                  this.var_998.push(this.var_2705);
               }
            }
            _loc8_--;
         }
         ...
         var _loc13_:ImageData = this.createUnionImage(this.var_998,_loc6_);
         var _loc14_:Point = new Point(-1 * _loc13_.regpoint.x,this.var_2706.y - _loc13_.regpoint.y);
         if(_loc6_)
         {
            if(this._scale == AvatarScaleType.const_57)
            {
         ...
         _loc8_ = this.var_998.length - 1;
         while(_loc8_ >= 0)
         {
            _loc15_ = this.var_998.pop();
            if(_loc15_)
            {
               _loc15_.dispose();
            }
