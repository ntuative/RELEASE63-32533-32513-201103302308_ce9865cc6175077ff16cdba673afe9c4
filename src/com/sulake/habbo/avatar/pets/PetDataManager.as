package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager, IDisposable
   {
      ...
      {
         if(param1 == null || this.var_520 == null)
         {
            return;
         }
         var _loc2_:int = this.var_520.indexOf(param1);
         if(_loc2_ != -1)
         {
            this.var_520.splice(_loc2_,1);
         }
      }
      ...
      {
         var _loc1_:* = null;
         while(this.var_520.length > 0)
         {
            _loc1_ = this.var_520.pop() as IPetDataListener;
            ...
            if(petData != null)
            {
               ...
               for each(breed in petData.breeds)
               {
                  ...
                  for each(targetSetName in targetSets)
                  {
                     ...
                     {
                        figurePart = figureTargetSet.RoomWidgetStoreSettingsMessage.part.(@id == petData.species).(hasOwnProperty("palettemapid") && @palettemapid == breed.patternId)[0];
                        if(figurePart == null)
                        {
                        }
      ...
      public function assetsReady(param1:int, param2:String, param3:IPetDataListener = null) : Boolean
      {
         ...
         {
            if(this.var_1468.indexOf(_loc6_) == -1)
            {
               _loc7_ = false;
               if(this.var_646.getValue(_loc6_) == null)
               {
                  ...
                  if(param3 != null && this.var_520.indexOf(param3) == -1)
                  {
                     this.var_520.push(param3);
                  }
               }
            }
         }
