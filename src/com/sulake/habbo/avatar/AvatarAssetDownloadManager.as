package com.sulake.habbo.avatar
{
   ...
   public class AvatarAssetDownloadManager extends EventDispatcher
   {
      ...
      private const const_2060:int = 100;
      ...
      public function AvatarAssetDownloadManager(param1:IAssetLibrary, param2:String, param3:String, param4:AvatarStructure)
      {
         ...
         this.var_922 = new Timer(this.const_2060,1);
         this.var_922.addEventListener(TimerEvent.TIMER_COMPLETE,this.onNextDownloadTimeout);
      }
      ...
      private function addMapLoaderEventListeners() : void
      {
         if(this.var_431)
         {
            this.var_431.addEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
            this.var_431.addEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function removeMapLoaderEventListeners() : void
      {
         if(this.var_431)
         {
            this.var_431.removeEventListener(AssetLoaderEvent.const_29,this.onConfigurationComplete);
            this.var_431.removeEventListener(AssetLoaderEvent.const_48,this.onConfigurationError);
         }
      }
      ...
      private function onConfigurationError(param1:Event) : void
      {
         ...
         {
            Logger.log("Could not download figuremap.xml. Retries left: " + this.var_921);
            if(this.var_1409.indexOf("?") > 0)
            {
               _loc2_ = this.var_1409 + "&retry=" + this.var_921;
            }
            else
            {
               _loc2_ = this.var_1409 + "?retry=" + this.var_921;
      ...
      private function generateMap(param1:XML) : void
      {
         ...
         for each(_loc2_ in param1.lib)
         {
            ...
            for each(_loc4_ in _loc2_.part)
            {
               ...
               _loc6_ = this.var_1123[_loc5_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(_loc3_);
               this.var_1123[_loc5_] = _loc6_;
            }
         }
      }
      ...
      public function loadFigureSetData(param1:IAvatarFigureContainer, param2:IAvatarImageListener) : void
      {
         ...
         if(_loc4_.length > 0)
         {
            ...
            {
               _loc6_ = this.var_520[_loc3_];
               if(_loc6_ == null)
               {
                  _loc6_ = [];
               }
               _loc6_.push(param2);
               this.var_520[_loc3_] = _loc6_;
            }
      ...
      private function libraryComplete(param1:Event) : void
      {
         ...
         for(_loc3_ in this.var_923)
         {
            ...
            if(_loc7_)
            {
               ...
               delete this.var_520[_loc3_];
            }
         }
         for each(_loc3_ in _loc2_)
         {
            delete this.var_923[_loc3_];
         }
         while(_loc6_ < this.var_743.length)
         {
            _loc5_ = this.var_743[_loc6_];
            if(_loc5_.libraryName == (param1.target as AvatarAssetDownloadLibrary).libraryName)
            {
               this.var_743.splice(_loc6_,1);
            }
            _loc6_++;
         }
         this.var_922.start();
      ...
      {
         var _loc1_:* = null;
         while(this.var_742.length > 0 && this.var_743.length < this.const_1636)
         {
            _loc1_ = this.var_742[0];
            _loc1_.startDownloading();
            this.var_743.push(this.var_742.shift());
         }
      }
      
      private function addToQueue(param1:AvatarAssetDownloadLibrary) : void
      {
         if(!param1.isReady && this.var_742.indexOf(param1) == -1 && this.var_743.indexOf(param1) == -1)
         {
