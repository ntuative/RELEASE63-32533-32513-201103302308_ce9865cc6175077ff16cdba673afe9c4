package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      private var var_432:IFriendCategoriesDeps;
      ...
      {
         var _loc2_:int = !!param1.online ? int(param1.categoryId) : int(FriendCategory.const_352);
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         try
         {
            ...
            status = 2;
            for each(removedFriendId in p.include)
            {
            ...
            for each(updatedFriend in p.updatedFriends)
            {
               ...
               this.var_432.messenger.setFollowingAllowed(updatedFriend.id,updatedFriend.followingAllowed && updatedFriend.online);
               currentlyOnline = this.isFriendOnline(updatedFriend.id);
               if(currentlyOnline && !updatedFriend.online)
               {
                  this.var_432.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_432.notifications.addOfflineNotification(updatedFriend.name,updatedFriend.realName);
               }
               if(!currentlyOnline && updatedFriend.online)
               {
                  this.var_432.messenger.setOnlineStatus(updatedFriend.id,updatedFriend.online);
                  this.var_432.view.setNewMessageArrived();
                  this.var_432.notifications.addOnlineNotification(updatedFriend.name,updatedFriend.figure,updatedFriend.realName);
               }
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         this.findCategory(FriendCategory.const_352).received = true;
         this.findCategory(FriendCategory.const_1044).received = true;
